...
object SpaceLoggerCommand {
    ...
    private fun executeTeleport(source: CommandSourceStack, dimensionName: String, x: Int, y: Int, z: Int): Int {
        ...
        player.teleportTo(targetLevel, x + 0.5, y.toDouble(), z + 0.5, player.yRot, player.xRot)
        return 1
    }
        ...
        line.append(
            Component.literal(relative).withStyle { style ->
                style.withColor(ChatFormatting.AQUA)
                    .withHoverEvent(HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal(fullTimestamp)))
            },
        )
        line.append(Component.literal(" "))
        ...
        line.append(
            Component.literal(coordText).withStyle { style ->
                style.withColor(ChatFormatting.GREEN)
                    .withHoverEvent(HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal("点击可传送")))
            ...
            if (row.verb() == NativeSpaceLoggerBridge.VERB_COMMAND) {
                ...
                line.append(
                    Component.literal("(cmd)").withStyle { style ->
                        style.withColor(ChatFormatting.DARK_GRAY)
                            .withHoverEvent(HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal(String(row.dataHead()))))
                    },
                )
            } else {
                line.append(Component.literal(" "))
                line.append(
                    Component.literal("(+${row.dataLen()})").withStyle { style ->
                        style.withColor(ChatFormatting.DARK_GRAY)
                            .withHoverEvent(HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal("nbt数据大小: ${row.dataLen()} bytes")))
                    },
                )
            }
        }
    ...
    private fun formatObjectComponent(objectRaw: String?): MutableComponent {
        ...
        val objectId = ResourceLocation.tryParse(objectRaw) ?: return Component.literal(objectRaw)
        return when {
            BuiltInRegistries.ITEM.containsKey(objectId) ->
                Component.translatable(BuiltInRegistries.ITEM.get(objectId).descriptionId)
            ...
            BuiltInRegistries.ENTITY_TYPE.containsKey(objectId) ->
                Component.translatable(BuiltInRegistries.ENTITY_TYPE.get(objectId).descriptionId)
    ...
    private fun parseDimensionIdentifier(dimensionName: String?): ResourceLocation {
        if (dimensionName.isNullOrBlank()) {
            throw syntax("dimension must not be empty")
        }
        return ResourceLocation.tryParse(dimensionName)
